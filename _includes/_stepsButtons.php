...
<div class="row">
    <div class="col-12">
        <ul class="list-style-none p-0 mt-4 pt-2 pb-3 list-steps d-flex justify-content-center">
            <?php for($i=1;$i<=6;$i++) : ?>
            <li class="<?= ($i === $stepCurrent) ? 'active' : ''; ?> <?= ($i < $stepCurrent) ? 'actived' : ''; ?>">
                <a href="4.<?=$i;?>-cadastro" class="ff-cond d-flex justify-content-center align-items-center fs-26 fw-500 text-dark"><?=$i;?></a>
            </li>
            <?php endfor; ?>
        </ul>
